...
import { cn, formatPrice, getInventoryPricing, unitThumbnailSrc } from '@/lib/utils';
...
export function TextUsDialog({ open, onOpenChange, unit }: TextUsDialogProps) {
  const { isAvailable, users } = useViewProWidget();
  ...
  const fetchChatResponse = useCallback(async (message: string) => {
    const res = (await api.post('chatgpt/chat', { message, body: 'class-b' })) as ChatGPTResponse;
    const { reply, inventories = [] } = res.data;
    return { reply, inventories: inventories.map(mapInventoryItem) };
  }, []);
